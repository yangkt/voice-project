...
class TIMITDataSet(Dataset):
    ...
    def __init__(self, TIMITRootPath, dialects, phoneDict, dataCnt=5000):
        ...
        self.fileCnt = len(self.trainSet)
        #
        ...
        for i in range(0, self.fileCnt):
            #
            fileName = self.trainSet.loc[i].wav.replace('\\\\', '\\')
            ...
            #
            fileName = self.trainSet.loc[i].phn.replace('\\\\', '\\')
...
def GenerateTIMITTrainingSetDict():
    ...
    trainSet = training_directory_loader(TIMITRoot, trainingFileName)
